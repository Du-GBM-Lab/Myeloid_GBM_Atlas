...
write.table(cellAnnota, "./CNV_Early/groupFiles.txt", sep = '\t', col.names = FALSE)
...
infercnv_obj = CreateInfercnvObject(raw_counts_matrix=exprMatrix,#单细胞Counts矩阵 
                                    annotations_file="./CNV_Early//groupFiles.txt",#分组文件(细胞) 
                                    delim="\t", 
                                    gene_order_file= "./CNV_Early/cnv_ref/mouse_gencode.GRCm39.vM32.basic.annotation.by_gene_name.infercnv_positions",#基因位置文件 
...
infercnv_obj = infercnv::run(infercnv_obj,#inferCNV对象 
                             ...
                             out_dir= './CNV_Early/' , # dir is auto-created for storing outputs 
...
# 3. 克隆识别 ----
# 获取CNV矩阵
load('./CNV_Early/infercnv_obj-hclust.RData')
...
ggsave('Clone Composition by Cell_Early.pdf',plot = pheatmap,width = 6, height = 4, units = "in")
...
write.csv(da_results, 'result_Early.csv', quote=F)
...
ggsave('plotDAbeeswarm_Early.pdf',plot = p,width = 5.2, height = 3.8, units = "in")
